...
import contacts.models.ContactManager;
...
public class Main {
  ...
  public static void main(String[] args) {
    try {
      loadContacts(
        "D:\\Dong\\tranning\\java\\JavaDevelopmentBootcamp\\sessionEight\\contacts\\contacts.txt"
      );
  ...
  public static void manageContacts() {
    ...
    while (true) {
      ...
      if (response.equals("a")) {
        ...
        if (
          name.isBlank() || phoneNumber.isBlank() || phoneNumber.length() < 5
        ) {
          System.out.println(
  ...
  public static void loadContacts(String fileName)
    ...
    while (scanFile.hasNextLine()) {
      try {
        contacts.models.Contact contact = new contacts.models.Contact(
          scanFile.next(),
          scanFile.next(),
          scanFile.next()
        );
